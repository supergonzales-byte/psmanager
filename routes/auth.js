...
router.post('/auth', async (req, res) => {
    ...
    if (!ldapCfg.enabled) return res.json({ ok: false, error: 'Authentification LDAP désactivée' })
    ...
    if (!inGroup) return res.json({ ok: false, error: 'Accès refusé — vous n\'êtes pas membre du groupe autorisé' })
