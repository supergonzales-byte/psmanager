...
const LOG = (...a) => console.log('[winget]', ...a)
...
async function getLatestVersion(packageId) {
    LOG(`getLatestVersion('${packageId}') — DÉBUT`)
    ...
    LOG(`getLatestVersion('${packageId}') → ${latest}`)
    return latest
}
...
async function downloadInstaller(packageId, version, destDir) {
    LOG(`downloadInstaller('${packageId}', '${version}') — DÉBUT`)
    if (!fs.existsSync(destDir)) fs.mkdirSync(destDir, { recursive: true })

    // 1. Lister les fichiers du dossier de version
    const versionPath = `${idToManifestPath(packageId)}/${version}`
    LOG(`étape 1/3 — lecture manifest : ${versionPath}`)
    const entries = await githubGet(versionPath)

    // 2. Trouver, télécharger et parser le .installer.yaml
    const installerEntry = entries.find(e =>
        e.type === 'file' && e.name.toLowerCase().endsWith('.installer.yaml'))
    if (!installerEntry) {
        throw new Error(`Fichier installer.yaml introuvable pour ${packageId}@${version}`)
    }
    LOG(`étape 2/3 — parsing ${installerEntry.name}`)
    ...
    LOG(`étape 2/3 ✓ arch=${best.Architecture}, type=${best.InstallerType}`)
    LOG(`           url=${best.InstallerUrl}`)

    // 3. Déterminer le nom du fichier final
    ...
    LOG(`étape 3/3 — téléchargement → ${filename}`)
    await psDownload(best.InstallerUrl, finalPath, 600)

    if (best.InstallerSha256) {
        LOG('vérification SHA256…')
        await verifySha256(finalPath, best.InstallerSha256)
        LOG('SHA256 ✓')
    }
